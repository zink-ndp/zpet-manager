  ...
  return <>
    <main className="w-full">
        <div className='flex flex-row z-10 w-full'>
            <div className=
                {'lg:flex relative w-[auto] lg:w-56 z-30 bg-white p-4 m-4 rounded-xl shadow-xl flex-col lg:items-start items-center space-y-5'} >
                <p className="hidden lg:blocktext-black text-2xl font-bold self-center py-4">ZPet</p>
                {(()=>{
                    const items:any = []
                    for (let index = 0; index < sidebarItems.length; index++) {
                        ...
                        items.push(<button className={activedTab == item.key ? 'sidebar-btn-active' : 'sidebar-btn'}
                                    ...
                                    >{item.icon}<p className="hidden lg:block">{item.text}</p></button>)
              ...
              <p className="hidden lg:block">Đăng xuất</p></button>
            </div>
